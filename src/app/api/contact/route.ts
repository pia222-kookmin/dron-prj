...
export async function POST(req: Request) {
  try {
    ...
    // 메일 옵션 설정
    const mailOptions = {
      from: `"${name}" <${gmailUser}>`, // Gmail SMTP는 발신 주소를 인증된 계정으로 고정하므로 표시 이름만 활용
      to: receiverEmail,
...
${message}
      ...
      html: `
        <div style="font-family: 'Malgun Gothic', 'Apple SD Gothic Neo', sans-serif; max-width: 600px; margin: 0 auto; padding: 30px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1e293b;">
          <h2 style="color: #0ea5e9; border-bottom: 2px solid #0ea5e9; padding-bottom: 15px; margin-top: 0;">📬 새로운 홈페이지 문의가 도착했습니다</h2>
          
          <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
            <tr>
              <td style="padding: 8px 0; font-weight: bold; width: 100px; color: #64748b;">이름</td>
          ...
          <div style="margin-top: 25px; padding: 20px; background-color: #f8fafc; border-left: 4px solid #0ea5e9; border-radius: 6px;">
          ...
          <div style="margin-top: 35px; font-size: 12px; color: #94a3b8; text-align: center; border-top: 1px solid #f1f5f9; padding-top: 20px;">
            본 메일은 국민대학교 차세대 드론 엔진 기술 홈페이지의 문의 접수 기능을 통해 자동 생성되어 발송되었습니다.
          </div>
        </div>
      `,
    };
