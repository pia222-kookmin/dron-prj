"use client";

import { FOOTER_CONTENT, SITE_CONFIG } from "@/constants";

interface FooterProps {
  lang?: "ko" | "en";
}

export default function Footer({ lang = "ko" }: FooterProps) {
  const t = FOOTER_CONTENT[lang] || FOOTER_CONTENT.ko;
  const description = lang === "ko" ? "고성능 드론 엔진 개발 및 대여 서비스" : "High-performance drone engine development and rental services.";
  const address = SITE_CONFIG.contact.address[lang] || SITE_CONFIG.contact.address.ko;

  return (
    <footer className="relative bg-dark-950 border-t border-dark-800">
      <div className="section-container py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          {/* 회사 정보 */}
          <div>
            <h3 className="font-display font-bold text-2xl text-white mb-4">
              {SITE_CONFIG.name}
            </h3>
            <p className="text-gray-500 text-sm mb-4 leading-relaxed">
              {description}
            </p>
            <div className="flex gap-4">
              {FOOTER_CONTENT.social.map((social) => (
                <a
                  key={social.platform}
                  href={social.href}
                  className="w-10 h-10 border border-dark-700 rounded-tech flex items-center justify-center text-gray-500 hover:border-cyber-500 hover:text-cyber-400 transition-colors"
                  aria-label={social.platform}
                >
                  <SocialIcon platform={social.platform} />
                </a>
              ))}
            </div>
          </div>

          {/* 연락처 */}
          <div>
            <h4 className="font-mono uppercase tracking-wider text-cyber-400 text-sm mb-4">
              Contact
            </h4>
            <div className="space-y-2 text-sm text-gray-500">
              <p>{SITE_CONFIG.contact.email}</p>
              <p>{SITE_CONFIG.contact.phone}</p>
              <p className="text-xs">{address}</p>
            </div>
          </div>

          {/* 링크 */}
          <div>
            <h4 className="font-mono uppercase tracking-wider text-cyber-400 text-sm mb-4">
              Legal
            </h4>
            <div className="space-y-2">
              {t.links.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="block text-sm text-gray-500 hover:text-cyber-400 transition-colors"
                >
                  {link.label}
                </a>
              ))}
            </div>
          </div>
        </div>

        {/* 저작권 */}
        <div className="pt-8 border-t border-dark-800 text-center">
          <p className="text-sm text-gray-600 font-mono">
            {t.copyright}
          </p>
        </div>
      </div>
    </footer>
  );
}

function SocialIcon({ platform }: { platform: string }) {
  const iconClass = "w-5 h-5";

  switch (platform) {
    case "github":
      return (
        <svg className={iconClass} fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
        </svg>
      );
    case "linkedin":
      return (
        <svg className={iconClass} fill="currentColor" viewBox="0 0 24 24">
          <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
        </svg>
      );
    case "twitter":
      return (
        <svg className={iconClass} fill="currentColor" viewBox="0 0 24 24">
          <path d="M23 3a10.9 10.9 0 01-3.14 1.53 4.48 4.48 0 00-7.86 3v1A10.66 10.66 0 013 4s-4 9 5 13a11.64 11.64 0 01-7 2c9 5 20 0 20-11.5a4.5 4.5 0 00-.08-.83A7.72 7.72 0 0023 3z" />
        </svg>
      );
    default:
      return null;
  }
}
